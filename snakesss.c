...
typedef enum {
    ST_MENU=0, ST_PLAYING, ST_RANKING, ST_INSTRUCTIONS, ST_GAMEOVER, ST_EXIT
} State;
...
void renderMenu() {
    ...
    int cols[] = { C_FIRE, C_HDR, C_GOOD, C_DIM };
...
void inputMenu(State *st) {
    ...
    if (k==13) {
        ...
        else if (menuIndex==1) { needRedrawRanking=1; *st=ST_RANKING; }
        else if (menuIndex==2) { needRedrawInstructions=1; *st=ST_INSTRUCTIONS; }
        else                   { *st=ST_EXIT; }
    }
    if (k==27) *st=ST_EXIT;
}
...
void inputRanking(State *st) {
    if (!_kbhit()) return;
    int k = _getch();
    if (k==13||k==27||k=='m'||k=='M') { needRedrawRanking=1; needRedrawMenu=1; *st=ST_MENU; }
}
...
void inputInstructions(State *st) {
    if (!_kbhit()) return;
    int k = _getch();
    if (k==13||k==27||k=='m'||k=='M'||k=='q'||k=='Q') { needRedrawMenu=1; *st=ST_MENU; }
}

void renderInstructions() {
    if (!needRedrawInstructions) return;
    clearScreen();
    int midX = CON_COLS / 2;
    ...
    setColor(14);
    gotoxy(midX - 16, 1);
    printf("%s  MANUAL DO JOGO - SNAKE INFERNO  %s", SYM_SNAKE, SYM_SNAKE);
    setColor(C_FIRE);
    gotoxy(4, 2);
    for (int i = 0; i < CON_COLS - 8; i++) printf("%s", BOX_DH);
    ...
    setColor(C_HDR);
    gotoxy(6, 4);
    printf("%s  OBJETIVO", SYM_STAR);
    setColor(C_NORMAL);
    gotoxy(6, 5);
    printf("Guie a cobra para comer frutas sem colidir com paredes,");
    gotoxy(6, 6);
    printf("obstaculos ou o proprio corpo. Quanto mais comer, maior fica!");

    /* Secao: Controles */
    setColor(C_HDR);
    gotoxy(6, 8);
    printf("%s  CONTROLES", SYM_CTRL);
    setColor(C_NORMAL);
    gotoxy(8, 9);  printf("W / Seta Cima    ->  Mover para cima");
    gotoxy(8, 10); printf("S / Seta Baixo   ->  Mover para baixo");
    gotoxy(8, 11); printf("A / Seta Esq.    ->  Mover para esquerda");
    gotoxy(8, 12); printf("D / Seta Dir.    ->  Mover para direita");
    gotoxy(8, 13); printf("P                ->  Pausar / Retomar");
    gotoxy(8, 14); printf("Q                ->  Voltar ao Menu");

    /* Secao: Itens do jogo */
    setColor(C_HDR);
    gotoxy(6, 16);
    printf("%s  ITENS DO JOGO", SYM_FIRE);

    setColor(12);
    gotoxy(8, 17); printf("%s  MACA       ", SYM_APPLE);
    setColor(C_NORMAL);
    printf("+10 pts (multiplicado pelo combo atual)");

    setColor(13);
    gotoxy(8, 18); printf("%s  DIAMANTE   ", SYM_DIAMOND);
    setColor(C_NORMAL);
    printf("+50 pts (aparece raramente, some em 15s!)");

    setColor(8);
    gotoxy(8, 19); printf("%s  POWER-UP   ", SYM_SKULL);
    setColor(C_NORMAL);
    printf("Frutos extras por 10s (ativa corpo colorido)");

    setColor(8);
    gotoxy(8, 20); printf("%s  ROCHA      ", SYM_ROCK);
    setColor(C_NORMAL);
    printf("Obstaculo fixo - colisao = fim de jogo!");

    /* Secao: Sistema de Pontos */
    setColor(C_HDR);
    gotoxy(6, 22);
    printf("%s  SISTEMA DE PONTOS E PROGRESSAO", SYM_ZAP);
    setColor(C_NORMAL);
    gotoxy(8, 23); printf("A cada 150 pts  ->  Power-Up spawna no mapa");
    gotoxy(8, 24); printf("A cada 200 pts  ->  Sobe de nivel (cobra mais rapida)");
    gotoxy(8, 25); printf("Combo           ->  Comer frutas seguidas multiplica os pontos");
    gotoxy(8, 26); printf("                    (max x8 - cuidado, decai se errar!)");

    /* Secao: Dicas */
    setColor(C_HDR);
    gotoxy(6, 28);
    printf("%s  DICAS", SYM_HEART);
    setColor(C_DIM);
    gotoxy(8, 29); printf("- Mantenha o combo ativo comendo frutas seguidas");
    gotoxy(8, 30); printf("- O Diamante vale muito mais com combo alto!");
    gotoxy(8, 31); printf("- Nos niveis altos, as rochas acumulam. Planeje a rota.");
    gotoxy(8, 32); printf("- Use a pausa (P) para pensar na jogada seguinte.");

    /* Rodape */
    setColor(C_DIM);
    gotoxy(midX - 20, 34);
    printf("Pressione ENTER, ESC ou Q para voltar ao menu principal");
    setColor(C_RESET);
    needRedrawInstructions = 0;
}
